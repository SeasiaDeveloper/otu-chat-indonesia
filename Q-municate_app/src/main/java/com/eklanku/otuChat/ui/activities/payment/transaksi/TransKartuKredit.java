...
//            Toast.makeText(this, "Masukkan minimal 8 digit nomor", Toast.LENGTH_SHORT).show();
    ...
    private void load_data() {
        loadingDialog = ProgressDialog.show(TransKartuKredit.this, "Harap Tunggu", "Mengambil Data...");
        loadingDialog.setCanceledOnTouchOutside(true);
        Log.d("OPPO-1", "cek_transaksi - Transkartukredit: "+FirebaseAuth.getInstance().getCurrentUser().getPhoneNumber());
...
//                        adapter = new ArrayAdapter<String>(getBaseContext(), android.R.layout.simple_list_item_1, nama_operator);
    ...
    private void cek_transaksi() {
        ...
        Log.d("OPPO-1", "cek_transaksi - transkartukredit: "+FirebaseAuth.getInstance().getCurrentUser().getPhoneNumber());
        Call<TransBeliResponse> transBeliCall = mApiInterface.postTransBeli(FirebaseAuth.getInstance().getCurrentUser().getPhoneNumber(), selected_operator, nominal, txtNo.getText().toString(), "kartubyr");
